...
describe('DiamondTest', async function () {
  ...
  before(async function () {
    ...
    facet3 =  await ethers.getContractAt('Facet3', diamondAddress)
  ...
  it('selectors should be associated to facets correctly -- multiple calls to facetAddress function', async () => {
    ...
    assert.equal(
      addresses[3],
      await diamondLoupeFacet.facetAddress('0xc47f0027')
    )
  })
  ...
  it('add most functions and facets', async () => {
    ...
    // single transaction
     const cut = [
      ...
      },
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(Facet3)
      },
      {
    ...
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[3], facets)][1], getSelectors(facet3))
    ...
    //
  it('set name in facet3', async () => {
    const Facet3 = await ethers.getContractAt('Facet3', diamondAddress)
    await Facet3.setName("slava");
  })

  it('read name from facet3', async () => {
    const Facet3 = await ethers.getContractAt('Facet3', diamondAddress)
    expect(await Facet3.getName()).to.equal('slava');
  })

  it('deploy facet3 v2', async () => {
    const Facet3v2 = await ethers.getContractFactory('Facet3v2');
    facet3v2 = await Facet3v2.deploy();
    await facet3v2.deployed();
    console.log(`facet3v2  deployed: ${facet3v2.address}`);
  })

  it('remove facet3 selectors', async () => {
    const Facet3v2 = await ethers.getContractFactory('Facet3v2');
    const selectors = getSelectors(facet3)
    tx = await diamondCutFacet.diamondCut(
      [{
        facetAddress: ethers.constants.AddressZero,
        action: FacetCutAction.Remove,
  ...
  it('add facet3v2', async () => {
    const selectors = getSelectors(facet3v2)
    await diamondCutFacet.diamondCut(
      [{
        facetAddress: facet3v2.address,
        action: FacetCutAction.Add,
        functionSelectors: selectors
      }],
  ...
  it('read name from facet3v2', async () => {
    const Facet3 = await ethers.getContractAt('Facet3', diamondAddress)
    expect(await Facet3.getName()).to.equal('slava');
  })
})